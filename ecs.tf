...
resource "aws_ecs_service" "conta-service-main" {
  ...
  depends_on = [aws_alb_listener.conta_app, aws_iam_role_policy_attachment.ecs_task_execution_role, aws_alb.conta_app, aws_db_instance.db_instance]
}
...
resource "aws_ecs_task_definition" "product-task-app" {
  ...
  depends_on = [aws_alb.product_app]
}
...
resource "aws_ecs_service" "product-service-main" {
  ...
  depends_on = [aws_alb_listener.product_app, aws_iam_role_policy_attachment.ecs_task_execution_role, aws_alb.product_app, aws_db_instance.db_instance]
}
